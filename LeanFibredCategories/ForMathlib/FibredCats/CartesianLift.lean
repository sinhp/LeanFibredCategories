...
@[simp]
def castIdComp  {c d : C} {f : c ⟶ d} {x : P⁻¹ c} {y : P⁻¹ d} : (x ⟶[(𝟙 c) ≫ f] y) ≃ (x ⟶[f] y)  := cast (id_comp f)

/-- Casting equivalence along precomposition with the identity morphism. -/
@[simp]
def castCompId  {c d : C} {f : c ⟶ d} {x : P⁻¹ c} {y : P⁻¹ d} : (x ⟶[f ≫ (𝟙 d) ] y)  ≃ (x ⟶[f] y) := cast (comp_id f)
...
lemma assoc_inv {c' c d d' : C} {f₁ : c' ⟶ c} {f₂ : c ⟶ d} {f₃ : d ⟶ d'} {w : P⁻¹ c'} {x : P⁻¹ c} {y : P⁻¹ d} {z : P⁻¹ d'} (g₁ : w ⟶[f₁] x) (g₂ : x ⟶[f₂] y) (g₃ : y ⟶[f₃] z) :  castAssoc.toFun ((g₁ ≫[l] g₂) ≫[l] g₃) =  (g₁ ≫[l] (g₂ ≫[l] g₃)) := by simp only [comp, Category.assoc, castAssoc, cast]
...
(g'' : x'' ⟶[u' ≫ u ≫ f] y) :
BasedLift.comp (gaplift  (g:= g') u' g'') (gaplift (g:= g) u g') =
gaplift (g:= g) (u' ≫ u) (BasedLift.castAssoc.invFun g'') := by
  refine gaplift_uniq (f:= f) g (BasedLift.castAssoc.invFun g'') (BasedLift.comp (gaplift  (g:= g') u' g'') (gaplift (g:= g) u g')) (by rw [BasedLift.assoc]; simp only [gaplift_property])
...
/-- The gapmap of a morphism `g'` with respect to a cartesian morphism `g` is the unique morphism `l` such that `l ≫ g = g'`. -/
noncomputable
...
/-- The identity morphism is cartesian. -/
...
lemma cart_comp : StableUnderComposition (CartMor P) := fun x y z f g hf hg w u g' => by
  ...
  · intro v hv
    have : v.hom ≫ f = lg.hom := (BasedLift.comp_hom_aux).mp (hlg.2 (v ≫[l] f) (hv ▸ assoc v.hom f g))
    apply hlf.2 v this
...
open BasedLift CartesianBasedLift
